...
class Store{
	...
	public Store(int _sid, int _storesNum, double _increRatio) {
		this.storeId = _sid;
		this.storesProbArr = new double[Parameter.moveRange*2][2]; //[n][0] : 다음 스토어, [n][1] : 다음 스토어로 갈 확률
		int[][] candidateArr = new int[Parameter.moveRange*2][2];	// [0] 후보자 , [1] 방문했었는지
		
		int n = _sid-Parameter.moveRange;
		for(int i=0; i<candidateArr.length; ) {
			if(n < 0) {
				candidateArr[i][0] = _storesNum + n++;
				candidateArr[i][1] = -1;
				i++;
			}else if(n!=_sid) {
				candidateArr[i][0] = n++%_storesNum;
				candidateArr[i][1] = -1;
				i++;
			}else
				n++;
		...
		for(int i=0; i<storesProbArr.length;) {
			int idx = (int) (Math.random()*storesProbArr.length);
			if(storesProbArr[idx][0] == -1 && candidateArr[i][1] == -1) {
				storesProbArr[idx][0] = candidateArr[i][0];
				candidateArr[i++][1] = 1;
			}
		}
